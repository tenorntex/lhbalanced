...
extern ktime_t dma_s_wait_time;

extern uint32 mddi_in_processing;
...
static void mdp_vsync_handler(void *data)
{
	...
	if (mfd->use_mdp_vsync) {
		...
		if (mfd->panel_info.type == MDDI_PANEL) {
			if (dma_s_wait_time.tv.nsec)
				hrtimer_start(&mfd->dma_hrtimer, dma_s_wait_time, HRTIMER_MODE_REL);
			else
...
void mdp_config_vsync(struct msm_fb_data_type *mfd)
{
	...
	if ((mfd->dest != DISPLAY_LCD) || (mfd->panel_info.pdest != DISPLAY_2)
	    || (!vsync_mode)) {
		goto err_handle;
	}
	...
	if (mfd->panel_info.lcd.vsync_enable) {
		...
		hrtimer_init(&mfd->dma_hrtimer, CLOCK_MONOTONIC,
			     HRTIMER_MODE_REL);
		mfd->dma_hrtimer.function = mdp_dma_s_vsync_hrtimer_handler;
		...
		if (mfd->panel_info.lcd.hw_vsync_mode) {
			...
			if (!mfd->use_mdp_vsync) {
				...
				set_irq_flags(mfd->channel_irq, IRQF_VALID | IRQF_NOAUTOEN);

				if (request_irq
				    (mfd->channel_irq,
				     &mdp_hw_vsync_handler_proxy,
				     IRQF_TRIGGER_HIGH, "VSYNC_GPIO",
				     (void *)mfd)) {
					MSM_FB_INFO
